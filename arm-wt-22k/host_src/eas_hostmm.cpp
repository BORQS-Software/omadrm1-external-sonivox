    ...
    for (i = 0; i < EAS_MAX_FILE_HANDLES; i++)
    {
        ...
        if (file->fd < 0)
        {
            ...
            if(drmManagerClient != NULL) {
                decryptHandle = drmManagerClient->openDecryptSession(fd,0,1);
                if (decryptHandle !=NULL) {
                    ALOGE("drm decryptHandle created");
                    char* data = new char[4096];
                    const char* tmpFile = "/data/local/.Drm/svtemp";
                    int decodedFD = -1;
                    remove(tmpFile); //remove file.
                    decodedFD = open (tmpFile, O_WRONLY | O_CREAT, 0777);                    
                    ALOGE("open %s for writing returnd fd=%d",tmpFile, decodedFD);
                    int offset = 0;
                    int len = 0;
                    ...
                    ALOGE("open %s in O_RDONLY mode returnd fd=%d",tmpFile, fd);
                }
